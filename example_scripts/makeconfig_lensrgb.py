...
epstio2=[round(mat_tio2(red),3),round(mat_tio2(green),3),round(mat_tio2(blue),3)]
epssio2=[round(mat_sio2(red),3),round(mat_sio2(green),3),round(mat_sio2(blue),3)]
...
f0=round(red/red,3)
f1=round(red/green,3)
f2=round(red/blue,3)
freq=[f0,f1,f2,f0,f1,f2]

#eps shape should be nspecs x 2*total_layers; for each spec, [eps_foreground,eps_background]
#ntot_layers=2*numlayers+1
eps=np.array([ ( epssio2[i%3], epssio2[i%3],
                 epstio2[i%3], epssio2[i%3],
                 epssio2[i%3], epssio2[i%3],
                 epstio2[i%3], 1,
                 1,1 ) for i in range(nspecs) ])  
print eps

nsub=[ round(np.sqrt(epssio2[i%3]),3) for i in range(nspecs) ]
polar=[0,0,0,0,0,0]
azimuth=[0,0,0,0,0,0]
