...
polar_=[10,10,10]
azimuth_=[0,0,0]
ax_=np.zeros((2,nspecs))
ay_=np.zeros((2,nspecs))
ax_[:,0]=[0,0]
ay_[:,0]=[1,0]
ax_[:,1]=[0,0]
ay_[:,1]=[1,0]
...
kx_=[ 2*np.pi*freq[i]*nsup[i]*np.sin(polar_[i]*np.pi/180)*np.cos(azimuth_[i]*np.pi/180) for i in range(nspecs) ]
ky_=[ 2*np.pi*freq[i]*nsup[i]*np.sin(polar_[i]*np.pi/180)*np.sin(azimuth_[i]*np.pi/180) for i in range(nspecs) ]
...
for i in range(nspecs):
    ...
    writeflag(fid,"-spec"+str(i)+"_out_kx,ky,ax,ay",[kx_[i],ky_[i],ax_[0,i],ax_[1,i],ay_[0,i],ay_[1,i]])
fid.close()
